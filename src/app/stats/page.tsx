    ...
    <div className="min-h-screen md:pl-20 lg:pl-64 pb-20 md:pb-0">
      ...
      <main className="max-w-4xl mx-auto px-6 pt-8 pb-16">
        ...
        <motion.div initial={{ opacity: 0, y: 16 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.1 }} className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-6">
          <BigStat value={stats.totalCards} label="Total de cards" icon={<CardsIcon />} color="#6366F1" />
          <BigStat value={`${stats.accuracy}%`} label="Precisão geral" icon={<TargetIcon />} color="#EF4444" />
          <BigStat value={stats.mastered} label="Dominados" icon={<StarIcon />} color="#F59E0B" />
          <BigStat value={`${stats.streak}d`} label="Sequência" icon={<FlameIcon />} color="#F97316" />
        </motion.div>
                ...
                {stats.byLanguage.map(({ lang, count }) => {
                  ...
                  return (
                    <div key={lang} className="flex items-center gap-3">
                      <span className="text-base w-6 text-center text-accent">{LANGUAGE_FLAGS[lang] ?? <GlobeIcon />}</span>
...
function BigStat({ value, label, icon, color }: { value: string | number; label: string; icon: React.ReactNode; color?: string }) {
  return (
    <div className="bg-surface border border-border rounded-2xl p-4">
      <div className="mb-2" style={{ color: color ?? "var(--accent)" }}>{icon}</div>
